...
from horoscope import calculate_porutham  # Import your new file
...
def clean_value(field, value):
    ...
    if field == "birth_time":
        # Fix common OCR misreads of "after"/"before"/"AM"/"PM" in birth time
        # e.g. OCR reads "after 0620" as "eter 0620" or "Afer 0620" etc.
        TIME_OCR_FIXES = [
            # "after" variants (OCR drops letters)
            (r"\b(eter|afer|afier|afler|afler|aftcr|aftec|afler|aier|afer|eter|eler|etcr)\b", "after"),
            # "before" variants
            (r"\b(befoe|befor|befoe|bef0re|bcfore|belore|befor|belore)\b", "before"),
            # AM/PM garbled
            (r"\b(4m|4M|Am|aM)\b", "AM"),
            (r"\b(pM|Pm|9m|9M)\b", "PM"),
        ]
        for pattern, replacement in TIME_OCR_FIXES:
            v = re.sub(pattern, replacement, v, flags=re.IGNORECASE)

        # Normalize format: "after 0620" → "After 06:20" if no colon
        def normalize_time_token(m):
        ...
        v = re.sub(r"\b(\d{3,4})\b", normalize_time_token, v)
        return v.strip()
...
class PoruthamRequest(BaseModel):
    bride_star_id: int  # 1 to 27
    groom_star_id: int  # 1 to 27

@app.post("/api/match/porutham")
def evaluate_porutham(data: PoruthamRequest):
    result = calculate_porutham(data.bride_star_id, data.groom_star_id)
    if "error" in result:

...
function RegisterStep1({ onNext, setPage, onOtpSent }: { onNext: (phone: string, gender: string) => void; setPage: (p: Page) => void; onOtpSent: (code: string) => void }) {
  ...
  const handleSendOtp = async () => {
    ...
    // Simulate registered number for demo based on design
    if (phone === "9876543210") {
...
function DashboardPage({ setPage, currentUser }: { setPage: (p: Page) => void; currentUser: any }) {
  ...
  const fetchProfiles = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/profiles?${params}`);
      const data = await res.json();
  ...
  const handleSendInterest = async (receiverId: number) => {
    if (!userId) return;
    const res = await fetch('http://localhost:5000/api/interests/send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function InterestsPage({ setPage, currentUser }: { setPage: (p: Page) => void; currentUser: any }) {
  ...
  const fetchInterests = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/interests?userId=${userId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleRespond = async (interestId: number, status: string) => {
    try {
      const res = await fetch('http://localhost:5000/api/interests/respond', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function ChatPage({ setPage, currentUser }: { setPage: (p: Page) => void; currentUser: any }) {
  ...
  const fetchHistory = async (receiverId: number) => {
    if (!userId) return;
    try {
      const res = await fetch(`http://localhost:5000/api/chats/${receiverId}?userId=${userId}`);
      const data = await res.json();
...
export default function App() {
  ...
  const refreshUser = async (userId?: number) => {
    ...
    if (!id) return;
    try {
      const res = await fetch(`http://localhost:5000/api/user/me?userId=${id}`);
      const data = await res.json();
      if (data.success) {

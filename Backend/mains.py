...
if os.name == "nt":
    ...
    # Automatically locate tesseract binary on Linux/Render containers
...
def extract_fields(text):
    ...
    # Search for candidate name explicitly, avoiding parent labels
    name_patterns = [

    # Tamil name (your case)
        r"(வெங்க[^\s]+)",

    # English name only (avoid random OCR words)
        r"\b([A-Z][a-z]{3,})\b"
    ]
    ...
    for pattern in name_patterns:
        ...
        # Ignore OCR garbage
            if (
                len(cleaned_val) > 2
                and not any(
                    word.lower() in cleaned_val.lower()
    ...
    # Birth Time matching (supports labels like Birth Time / जन्म समय / etc.)
        time_match = re.search(r"(?:Birth\s*Time|birthTime|பிறந்த\s*நேரம்)\s*[:：]?\s*(\b\d{1,2}[:.]\d{2}\s*(?:AM|PM|am|pm)?\b)|\b\d{1,2}[:.]\d{2}\s*(?:AM|PM|am|pm)?\b", text)
        
        fields["birthTime"] = time_match.group(1) or time_match.group(0) if time_match else ""

        fields["education"] = get_value(text, "கல்வித் தகுதி")
        fields["occupation"] = get_value(text, "பணி")
        fields["height"] = get_value(text, "உயரம்")
        fields["weight"] = get_value(text, "எடை")
        fields["fatherName"] = get_value(text, "தந்தை பெயர்")
        fields["motherName"] = get_value(text, "தாயார் பெயர்")

        rasi = get_value(text, "இராசி")
        fields["rasi"] = RASI_MAP.get(rasi, rasi)
        ...
        location_keywords = [
            "சென்னை", "மதுரை", "கோயம்புத்தூர்", "திருச்சி", "மும்பை",
            "Chennai", "Madurai", "Coimbatore", "Mumbai"
        ]
    ...
    fields["birthPlace"] = ""
    for key in birth_place_keywords:
...
async def extract_text(file: UploadFile = File(...)):
    ...
    for key, value in fields.items():
        ...
        if isinstance(value, str) and value.strip():
            try:
                fields[key] = translator.translate(value)
    ...
    "fields": {
        ...
        "birthTime": fields.get("birthTime", ""),
        ...
        "fatherName": fields.get("fatherName", ""),
        "motherName": fields.get("motherName", ""),
        "rasi": fields.get("rasi", ""),
        "phones": fields.get("phones", []),
        "city": fields.get("city", ""),
        "state": fields.get("state", "Tamil Nadu"),
...
#     # Exact Tamil name field
#     r"(?<!தாயார்\s)(?<!தந்தை\s)பெயர்\s*[:：]?\s*\n?\s*([^\n]+)",
...
#     if time_match:
#         fields["birthTime"] = time_match.group(0)
#     else:
#         fields["birthTime"] = ""
...
#         fields["education"] = get_value(text, "கல்வித் தகுதி")
#         fields["occupation"] = get_value(text, "பணி")
#         fields["income"] = get_value(text, "மாத வருமானம்")
#         fields["birthPlace"] = get_value(text, "பிறந்த இடம்")
#         fields["height"] = get_value(text, "உயரம்")
#         fields["weight"] = get_value(text, "எடை")
#         fields["fatherName"] = get_value(text, "தந்தை பெயர்")
#         fields["motherName"] = get_value(text, "தாயார் பெயர்")
